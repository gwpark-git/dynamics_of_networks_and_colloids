...
% &= \frac{1}{K(K-1)}\left[(K-1)N_b(m_K) - \sum_{k=1}^{K-1}N_b(m_k)\right].\label{eq:IDENT_EQ_one_step}
% \end{align}
...
def get_rdf_ref(traj, ts, dr, Np, N_dimension, box_dimension, cut_ratio):
    ...
    %% Again, the $\mathbf{r}_i\in \boldsymbol{\Gamma}_{1}^{N}$ which is set of position vectors. Therfore, for given $\mathbf{r}$, the integration part becomes
    %% \begin{equation}
    %%   \int d\mathbf{r}_i d\mathbf{r}_j \delta(\mathbf{r} - (\mathbf{r}_i - \mathbf{r}_j)) = \left\{\begin{array}{cc} 1 & \textrm{when }\mathbf{r}_i - \mathbf{r}_j = \mathbf{r} \\ 0 & \textrm{when }\mathbf{r}_i - \mathbf{r}_j \neq \mathbf{r} \end{array}\right.
    %% \end{equation}
    

      

    %% Same with the correlation function, let assumed the particles are identical. Then, we can 
    % The further expand gave us
    % \begin{align}
    %   S(\mathbf{k}) &= 1+N^{-1}N(N-1)\langle \exp(i\mathbf{k}\cdot(\mathbf{r}_1 - \mathbf{r}_2)\rangle \\
    %   &= 1 + N^{-1}N(N-1)\int d\boldsymbol{\Gamma}_{1}^{N} \exp(i\mathbf{k}\cdot(\mathbf{r}_1 - \mathbf{r}_2))\frac{\exp(-\beta U(\boldsymbol{\Gamma}_{1}^{N}))}{Z} \\
    %   &= 1 + N^{-1}N(N-1)\int d\mathbf{r}_1d\mathbf{r}_2 \left[\exp(i\mathbf{k}\cdot(\mathbf{r}_1-\mathbf{r}_2))\int d\boldsymbol{\Gamma}_{3}^{N}\frac{\exp(-\beta U(\boldsymbol{\Gamma}_{1}^{N}))}{Z}\right]\\
    %   &= 1 + N^{-1}N(N-1)
    % \end{align}
\end{appendices}
