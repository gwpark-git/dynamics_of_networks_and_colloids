...
# RF_NP1000 = loadtxt('../longer_computation/virial_stress/RF_NP1000_RT100_dt0_01_500td.dat')[N_st:,1]/normal_factor
# acf_NP1000 = acf_gro(RF_NP1000)
# t_NP1000 = arange(size(acf_NP1000))/100.

# RF_NP1200 = loadtxt('../longer_computation/virial_stress/RF_NP1200_RT100_dt0_01_longer.dat')[N_st:,1]/normal_factor
# acf_NP1200 = acf_gro(RF_NP1200)
# t_NP1200 = arange(size(acf_NP1200))/100.

# RF_NP1400 = loadtxt('../longer_computation/virial_stress/RF_NP1400_RT100_dt0_01_longer.dat')[N_st:,1]/normal_factor
...
plt.semilogy(t_NP0400, acf_NP0400, '+-', color=colP[0], label = r'$\nu_0=10/R_0^3$, $\lambda(t^{(1)})$ = %4.3f $\tau_0$, $\lambda(t^{(2)})$ = %4.3f $\tau_0$'%(-1./a_400, -1./a2_400))
# plt.semilogy(t_NP0600, acf_NP0600, '+-', color=colP[1], label = r'$\nu_0=15/R_0^3$, $\lambda(t^{(1)})$ = %4.3f $\tau_0$, $\lambda(t^{(2)})$ = %4.3f $\tau_0$'%(-1./a_600, -1./a2_600))
# plt.semilogy(t_NP0800, acf_NP0800, '+-', color=colP[2], label = r'$\nu_0=20/R_0^3$, $\lambda(t^{(1)})$ = %4.3f $\tau_0$, $\lambda(t^{(2)})$ = %4.3f $\tau_0$'%(-1./a_800, -1./a2_800))

plt.semilogy(t_NP1000, acf_NP1000, '+-', color=colP[3], label = r'$\nu_0=25/R_0^3$, $\lambda(t^{(1)})$ = %4.3f $\tau_0$, $\lambda(t^{(2)})$ = %4.3f $\tau_0$'%(-1./a_1000, -1./a2_1000))
plt.semilogy(t_NP1200, acf_NP1200, '+-', color=colP[4], label = r'$\nu_0=30/R_0^3$, $\lambda(t^{(1)})$ = %4.3f $\tau_0$, $\lambda(t^{(2)})$ = %4.3f $\tau_0$'%(-1./a_1200, -1./a2_1200))
plt.semilogy(t_NP1400, acf_NP1400, '+-', color=colP[5], label = r'$\nu_0=35/R_0^3$, $\lambda(t^{(1)})$ = %4.3f $\tau_0$, $\lambda(t^{(2)})$ = %4.3f $\tau_0$'%( -1./a_1400, -1./a2_1400))
...
y_min = 10**-7
y_max = 10**-2
plt.annotate(r'$t^{(1)}\in$(0, %3.2f)'%t_NP0400[N1], xy=(0.01, y_min*10))
plt.annotate(r'$t^{(2)}\in$(%3.1f, %3.1f)'%(t_NP0400[N2_st], t_NP0400[N2]), xy=(0.15, y_min*2))

ref_vert_1 = asarray([[t_NP0400[N1], y_min], [t_NP0400[N1], y_max]])
ref_vert_2 = asarray([[t_NP0400[N2_st], y_min], [t_NP0400[N2_st], y_max]])
ref_vert_3 = asarray([[t_NP0400[N2], y_min], [t_NP0400[N2], y_max]])
...
plt.ylabel('dimensionless stress autocorrelation function')
plt.legend(loc = 'lower right', prop=fontP, numpoints=1)
plt.grid()
plt.axis([0, 5, y_min, y_max])
