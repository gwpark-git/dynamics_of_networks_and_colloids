...
for i_NP, NP in enumerate(NP_arr):
    ...
    for RT in RT_arr:
        for alpha in alpha_arr:
            for lc in lc_arr:
                for NC in NC_arr:
                    print 'bash automation_submit_recasna.sh equilibrium_detail/C025/NP%s_%s_C025_%s%s_%s_%s.inp $N_runs $init_sample_number ../%sVER/EQ_NP%s_%s/NP%s_%s.traj'%(NP, LD, RT, alpha, lc, NC, LD, NP, LD, NP, LD)
