...
then
    ...
    cp stochastic_HEUR_recasna.sh $file_inp.job/stochastic_HEUR_recasna.sh
    arg_str=''
    if [ $# -gt 2 ]
    then
	for (( i=4; i<=$#; i++))
    ...
    python gen_job_files.py $file_inp $N_runs $init_seed_number stochastic_HEUR_recasna.sh $server $arg_str >> $file_inp.job/jobs.org
