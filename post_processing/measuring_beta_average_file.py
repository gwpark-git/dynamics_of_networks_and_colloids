                ...
                while(IDENTIFIER):
                    ...
                    try:
                        if (t_cnt >= minimum_time_index and (t_cnt < maximum_time_index or maximum_time_index == -1)):
                            ...
                            max_R = 0
                            for i in range(Np):
                                # note that the loop chain contribution is ignored from now on
                                # tmp_beta += connectivity[i, i]*1 # contribution for loop chain
                                ...
                                for j in range(i + 1, Np):
                                    if connectivity[i, j] > 0:
                                        ...
                                        single_beta = compute_beta(tmp_R, N_dimension, lc, alpha)
                                        if single_beta > max_beta:
                                            max_beta = single_beta
                                        tmp_beta += connectivity[i,j]*single_beta
                            ...
                            #     else:
                            #         dat.append([0., tmp_beta/float(tmp_cnt)])
                            # else:
                            #     dat.append([0., 0.])
                            if tmp_cnt <> 0:
                                dat.append([tmp_r2/float(tmp_cnt), tmp_beta/float(tmp_cnt), max_R, max_beta])
                            else:
                                dat.append([0., 0.])
                        elif (t_cnt >= maximum_time_index and maximum_time_index <> -1):
                            break
                        else:
                            # this is set when the minimum accounting time index
                            # pos = read_traj_step(f_traj, Np, N_dimension)
                            # connectivity = read_connectivity_step(f_index, f_weight, Np)
                            tmp_traj = f_traj.readline()
