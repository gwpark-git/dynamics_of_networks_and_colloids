...
def rep_force(R_vec, C_repulsion, N_dimension):
    R_sca = norm(R_vec)
    if R_sca < 1.0:
        return C_repulsion*(R_sca**2.0 - 1.0)*R_vec/R_sca
    return 0.

# def F_ij(R_vec, alpha, N_dimension, C_repulsion):
#     if C_repulsion <> 0.:
#         return int_Gaussian(R_vec, alpha, N_dimension) + rep_force(R_vec, C_repulsion, N_dimension)
#     return int_Gaussian(R_vec, alpha, N_dimension)
...
if size(sys.argv) < 8:
    ...
    print 'argv[6] == alpha'
    print 'argv[7] == TRUE for including repulsive contribution'
    print 'argv[8](OPTIONAL) == repulsive coefficient if it is TRUE'
    print 'argv[9](OPTIONAL) == initial cut (default is 1)'
else:
    ...
    C_repulsion = 0.
    if(sys.argv[7] == 'TRUE'):
        C_repulsion = float(sys.argv[8])
    RF = []
    try:
        initial_cnt = int(sys.argv[9])
    except:
        initial_cnt = 0
                            ...
                            for i in range(Np):
                                for j in range(i + 1, Np):
                                    if connectivity[i,j] > 0:
                                        ...
                                        if (C_repulsion > 0.):
                                            tmp_F_rep = rep_force(tmp_R, C_repulsion, N_dimension)
                                            tmp_RF += connectivity[i,j] * outer(tmp_R, tmp_F) + outer(tmp_R, tmp_F_rep)
                                        else:
