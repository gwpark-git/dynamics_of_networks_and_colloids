...
if size(sys.argv) < 11:
    ...
    print 'argv[4] == minimum of log10(q)'
    print 'argv[5] == maximum of log10(q)'
    ...
    fn = sys.argv[1]
    fn_out = sys.argv[2]
    cut_ident = float(sys.argv[3])
    q_log10_min = float(sys.argv[4])
    q_log10_max = float(sys.argv[5])
    q = 2.*pi*10**linspace(q_log10_min, q_log10_max, 1000)
    Sq_log10_min = float(sys.argv[6])
    Sq_log10_max = float(sys.argv[7])
    rho = float(sys.argv[8])
    given_max_cut = float(sys.argv[9])
    box_dimension = float(sys.argv[10])
    rdf_r11 = loadtxt(fn)
    r11 = rdf_r11[:,0]
    gr11 = rdf_r11[:,2]
