...
namespace INTEGRATOR
{
  namespace EULER
  {
    ...
    double cal_connector_force_boost(POTENTIAL_SET& POTs, CONNECTIVITY& CONNECT, MATRIX& given_vec, MKL_LONG given_index, MATRIX** R_minimum_vec_boost, MATRIX* R_minimum_distance_boost);
  }
...
namespace ANALYSIS
{
  ...
  namespace DUMBBELL
  {
    double CAL_ENERGY_R_boost(POTENTIAL_SET& POTs, CONNECTIVITY& CONNECT, MATRIX& mat_energy, double time, RDIST& R_boost); // note that this functionality is subect to re-define. It is due to the fact that RDIST is not necessary for the dumbbell model, but it is used for using the same interface with stochastic HEUR simulation
    double cal_potential_energy_R_boost(POTENTIAL_SET& POTs, CONNECTIVITY& CONNECT, RDIST& R_boost);
  }
  namespace ANAL_ASSOCIATION
  {
    double CAL_ENERGY_R_boost(POTENTIAL_SET& POTs, ASSOCIATION& CONNECT, MATRIX& mat_energy, double time, RDIST& R_boost);
    double cal_potential_energy_R_boost(POTENTIAL_SET& POTs, ASSOCIATION& CONNECT, RDIST& R_boost);
  }
}
