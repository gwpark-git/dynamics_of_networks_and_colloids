      ...
      for(MKL_LONG p=0; p<R_boost.TOKEN[cell_index_neighbor]; p++)
        {
          ...
          double distance = R_boost.measure_minimum_distance(TRAJ, index_t, index_particle, index_target, R_boost.BEYOND_BOX[cell_index_particle][k]);
  ...
  if(given_condition("SIMPLE_SHEAR") == "TRUE")
    {
      ...
      measure_minimum_distance = measure_minimum_distance_simple_shear;

    }
  else
    {
      if(CELL_LIST_BOOST)
        meausre_minimum_distance = measure_minimum_distance_cell_list;
      else
        measure_minimum_distance = measure_minimum_distance_default;
    }
}
